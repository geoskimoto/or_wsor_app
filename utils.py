...
def style_BFcst(BFcst, basin_name):
    ...
    s.set_properties(subset=slice_, **{'border-right': '1.3px solid #D3D3D3',
                                       'border-left': '1.4px solid #D3D3D3',
...
def style_Snow(BSnow, basin_name):
    ...
    # awesome line of code to round columns that have int and floats in them.  Only works for actual dataframe.  Actual solution is to use df.style()
...
def snowpack_parser(spreadsheet, basin_name):
    ...
    table = [i for i in table.items()][-1][1]
    table = table.iloc[:, :4]
